# correct program for wine production to run:   streamlit run s7.py
# st3_scenarios_full.py — Production Budget (Step 3) with two scenarios, compact headers, HTML report, and graph
# Updated: adds "set" mode (replace), Organic low-yield 41% band included, Fairtrade labour tweak excludes "Toesig en bestuurshulp"
...
st.set_page_config(
    page_title="Wine Grape Production Budget",
    layout="wide",
    initial_sidebar_state="expanded"
)

####  # --- Export section CSS (STREAMLIT UI) ---
st.markdown("""
<style>
.export-card{
  background:#fff;
...
.export-card .stDownloadButton > button{
  width:100%;
  padding:10px 14px;
  border-radius:10px;
}
...
# Optional: tighten spacing between sidebar and main content
st.markdown("""
<style>
...
# tighten spacing between sidebar and main content
st.markdown("""
<style>
[data-testid="stAppViewContainer"] .main .block-container {
  padding-left: 8px;   /* reduce from default ~2rem */
  padding-right: 8px;
}
...
[data-testid="stAppViewContainer"] .main {
  margin-left: -6px;   /* closes the gap */
}
section[data-testid="stSidebar"] {
  min-width: 290px;    /* optional: widen sidebar a bit */
}
</style>
""", unsafe_allow_html=True)
...
st.markdown("<p style='color:#6b7280; margin-top:-10px;'><em>A scenario-based model for benchmarking and decision support.</em></p>", unsafe_allow_html=True)

# st.subheader("How the Farm Compares with the Industry")
...
def load_data():
    ...
    income = norm(income); yield_df = norm(yield_df); costs = norm(costs)
    ...
    if "Avg_Cost" not in costs.columns:
        ...
        if g: costs = costs.rename(columns={g[0]: "Avg_Cost"})
        else: costs["Avg_Cost"] = 0.0
...
def money_r(x):    # with symbol (for other parts of app)
    return f"R {x:,.2f}"
...
with st.sidebar:
    ...
    wine_class = st.selectbox("Wine Class", wine_classes, index=(wine_classes.index("White") if "White" in wine_classes else 0))
    grapes = sorted(yield_df.loc[yield_df["Wine Class"] == wine_class, "Grape Variety"].dropna().unique().tolist())
    grape_variety = st.selectbox("Grape Variety", grapes, index=(grapes.index("Chenin blanc") if "Chenin blanc" in grapes else 0))
    regions = sorted(set(
        yield_df.loc[(yield_df["Wine Class"] == wine_class) & (yield_df["Grape Variety"] == grape_variety), "Region"]
    ).union(set(income_df.loc[(income_df["Wine Class"] == wine_class) & (income_df["Grape Variety"] == grape_variety), "Region"]))
...
aligned = key_all.merge(farmer_items, on=["Category","Item"], how="left").merge(avg_items, on=["Category","Item"], how="left")
...
def _apply_abs(x, inc): return (0.0 if pd.isna(x) else float(x)) + float(inc)
def _apply_pct(x, pct): return (0.0 if pd.isna(x) else float(x)) * (1.0 + float(pct)/100.0)
...
# --- DO NOT redefine SCENARIOS here ---
# SCENARIOS must stay your original dict-of-dicts, e.g.:
# SCENARIOS = {
#   "Scenario 1 — Organic": {"label":"Organic", "cost_rules": [...], ...},
#   "Scenario 2 — Fairtrade": {"label":"Fairtrade", "cost_rules": [...], ...},
#   "Scenario 3 — Organic + Fairtrade": {"label":"Organic + Fairtrade", "cost_rules": [...], ...},
# }

# 1) Options come from the existing keys:
...
# Costs header
# rows += [["Costs", "Farmer", "Industry Low", "Industry High", f"{SC_LABEL}"]]
...
for cat in aligned["Category"].unique():
    if is_forbidden(cat): continue
    f_total = float(cat_tot_farmer.loc[cat_tot_farmer["Category"]==cat, "Cost"].fillna(0).iloc[0]) if cat in cat_tot_farmer["Category"].values else 0.0
    a_total = float(cat_tot_avg.loc[cat_tot_avg["Category"]==cat, "Avg"].fillna(0).iloc[0]) if cat in cat_tot_avg["Category"].values else 0.0
    s_total = float(sc_cat.loc[sc_cat["Category"]==cat, "Value"].fillna(0).iloc[0]) if cat in sc_cat["Category"].values else 0.0
...
rows += [
    ...
    ["Total Expenditure",       money_n(total_expenditure_user),money_n(industry_total_expenditure),   money_n(SC["total_exp"])],
    ...
    <head>
      ...
      <style>
        ...
        .tbl td.allow-wrap{{
          ...
          overflow-wrap: anywhere;   /* break at spaces, slashes, or digits if needed */
        ...
        .profile-grid thead th {{ text-align:right; font-weight:700; color:#374151; background:#f8fafc; border:1px solid #e8e8e8; padding:6px 10px; }}
        /* Keep general header alignment right */
        ...
        .footer {{ margin-top:14px; padding-top:8px; border-top:1px solid #e6e6e6; text-align:center; font-size:0.82rem; color:#667; }}
      <style>
  /* Reduce the footer-to-next-section spacing */
  .footer {{
    margin-top: 4px !important;     /* tighten vertical gap */
    padding-top: 2px !important;
  }}

  /* Reduce white space before the next section title */
  .section-title {{
...
@media screen {{
  .footer:hover {{
    color: #374151;             /* darker gray when hovered */
  }}
}}

/* Print rule — ensure it stays visible and centered */
@media print {{
  .footer {{
...
components.html(build_html(table, SC_LABEL), height=740, scrolling=True)
...
def plot_net_summary_bar(sc_label: str, show_yield: bool = True):
    # --- NFI data (unchanged) ---
    data = net_summary_for_charts(sc_label)   # [("Farmer", ...), ("Industry Low", ...), ...]
    labels = [k for k, _ in data]
    values = [v for _, v in data]

    # Grouped color scheme
    color_map = {
        "Farmer": "#2ca02c",
        "Industry Low": "#9ca3af",
        "Industry High": "#6b7280",
        "Organic Low": "#93c5fd",
        "Organic High": "#3b82f6",
    }
    ...
    # Base colors for all bars (will be trimmed to length)
    base_palette = ["#9ca3af", "#4b5563", "#93c5fd", "#3b82f6"]
    colors = (["#9ca3af"] * len(values))  # neutral default for all
    # Make farmer conditional: green if ≥ 0, red if < 0
    colors[farmer_idx] = "#1CBA59" if farmer_nfi >= 0 else "#D64545"

    # Plot
    x = range(len(labels))
    ax.bar(x, values, color=colors, label="Net Farm Income (bars)")
    ...
    if show_yield:
        yield_map = {
            ...
            "Organic Low": SC.get("yield_low"),
            "Organic High": SC.get("yield_high"),
        }
    ...
    legend_handles = [
        Patch(facecolor="#9ca3af", edgecolor="#374151", label="Net Farm Income (bars)"),
    ...
    ax.legend(
        ...
        loc="upper center",
        bbox_to_anchor=(0.5, 1.10),   # small legend above the plot
        ncol=2,
        frameon=True,
        fancybox=True,
        framealpha=0.95,
    ...
    fig.subplots_adjust(top=0.85)     # make space for the legend

    fig.tight_layout()
    return fig

    # Clean title
    # ax.set_title("Competitiveness Snapshot", pad=20)
...
def build_range_plot():
    ...
    # Industry range (soft fill + edge)
    ax.fill_between([y0, y1], n0, n1, step="pre", alpha=0.6,
                    color=PALETTE["industry_fill"], zorder=1)
    ax.plot([y0, y0, y1, y1, y0], [n0, n1, n1, n0, n0],
            color=PALETTE["industry_edge"], linewidth=1.2, zorder=2, label="Industry range")
    ...
    if sy0 is not None and sy1 is not None and sn0 is not None and sn1 is not None:
        ax.plot([sy0, sy0, sy1, sy1, sy0], [sn0, sn1, sn1, sn0, sn0],
                color=PALETTE["scenario_edge"], linewidth=1.6, linestyle="--",
                zorder=2.5, label=f"{SC_LABEL} range")
    ...
    if in_range:
        ax.scatter(yield_current, net_current, s=68,
                   facecolor=sign_color, edgecolor="white", linewidths=1.0, zorder=3)
    else:
        ax.scatter(yield_current, net_current, s=78,
                   facecolor="none", edgecolor=sign_color, linewidths=2.0, zorder=3)

    ax.annotate("Farmer", (yield_current, net_current), xytext=(6, 10),
                textcoords="offset points", fontsize=9, color=sign_color, weight="600")
    ...
    ax.set_xlim(x_min - xspan*0.1, x_max + xspan*0.1)
    ax.set_ylim(y_min - yspan*0.15, y_max + yspan*0.15)

    ax.set_xlabel("Yield (t/ha)")
    ax.set_ylabel("Net Farm Income (R/ha)")
    # ax.set_title("Farmer vs Industry and Scenario Ranges", pad=45)
    ax.yaxis.set_major_formatter(FuncFormatter(lambda v, pos: f"R {v:,.0f}"))
    ax.grid(alpha=0.12)

    # Legend: show the two-state farmer marker + ranges
    legend_handles = [
        ...
        Line2D([0], [0], color=PALETTE["scenario_edge"], linestyle="--", linewidth=1.6,
               label=f"{SC_LABEL} range"),
        Line2D([0], [0], marker="o", linestyle="none", markersize=7,
               markerfacecolor=PALETTE["green_pos"], markeredgecolor="white",
               label="Farmer (in range, ≥0)"),
        Line2D([0], [0], marker="o", linestyle="none", markersize=7,
               markerfacecolor="none", markeredgecolor=PALETTE["green_pos"], linewidth=2.0,
               label="Farmer (out of range, ≥0)"),
        Line2D([0], [0], marker="o", linestyle="none", markersize=7,
               markerfacecolor=PALETTE["red_neg"], markeredgecolor="white",
               label="Farmer (in range, <0)"),
        Line2D([0], [0], marker="o", linestyle="none", markersize=7,
               markerfacecolor="none", markeredgecolor=PALETTE["red_neg"], linewidth=2.0,
               label="Farmer (out of range, <0)"),
    ]
    # Legend
    legend = ax.legend(
    ...
    fig.subplots_adjust(top=0.78)  # make space for legend above
    return fig
...
# industry band
y0, y1 = (min(yield_low, yield_high), max(yield_low, yield_high)) if (yield_low is not None and yield_high is not None) else (yield_current, yield_current)
n0, n1 = (min(net_low, net_high), max(net_low, net_high)) if (net_low is not None and net_high is not None) else (net_current, net_current)
...
# --- Export block (wrap your buttons) ---
# st.markdown('<div class="export-card">', unsafe_allow_html=True)
# st.markdown('<div class="export-header">Export</div>', unsafe_allow_html=True)
# st.markdown('<div class="export-sub">Download the current view.</div>', unsafe_allow_html=True)
...
def to_excel_bytes(df, fig_range=None, fig_bar=None):
    ...
    with pd.ExcelWriter(out, engine=engine) as writer:
        ...
        if engine == "xlsxwriter":
            ...
            row = 0
            ws2.write(row, 0, "Wine Class:"); ws2.write(row, 1, str(wine_class)); row += 1
            ws2.write(row, 0, "Grape Variety:"); ws2.write(row, 1, str(grape_variety)); row += 1
            ws2.write(row, 0, "Region:"); ws2.write(row, 1, str(region)); row += 1
            ws2.write(row, 0, "Scenario:"); ws2.write(row, 1, str(SC_LABEL)); row += 2
...
@media print {{
  /* Page setup */
  @page {{ size: A4; margin: 18mm 12mm 18mm 12mm; }} /* top right bottom left */
  ...
  /* Keep rows/cells intact; repeat headers on new pages */
  ...
  /* Small comfort tweaks */
  .tbl thead tr:first-child th {{ padding-top: 6mm !important; }} /* reduce if too large */
  .print-gap {{ height: 6mm; }} /* optional spacer before large tables */
}}
...
# Card wrapper (end)
st.markdown('</div></div>', unsafe_allow_html=True)   # closes .export-grid and .export-card
